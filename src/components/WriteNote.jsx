...
const WriteNote = ({onClose,showToast}) => {
  ...
  return (
    <>
    <div ref={modalRef} onClick={closeNote} className='w-screen h-screen fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm z-10'>
      <div className='mt-10 mx-[5rem] flex flex-col text-slate-700 bg-slate-700 py-10 px-10 bg-opacity-10 rounded-lg max-sm:px-0.5 max-sm:mx-[0.5rem] z-20'>
        <button className='place-self-end px-8' onClick={onClose}><XSquare size={40}/></button>
        <div className='flex flex-col gap-5 justify-center mx-auto w-[25rem] md:w-[34rem] lg:w-[45rem] xl:w-[60rem] max-sm:w-[18rem]'>
          ...
          <a onClick={()=>{
              ...
              axios.post('https://note-taking-app-backend-wpdj.onrender.com/v1/notes/',{
                title:title,
